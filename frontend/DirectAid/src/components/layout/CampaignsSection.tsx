...
const CampaignCard: React.FC<CampaignCardProps> = ({ id: _id, title, description, target, raised, imageUrl }) => {
  const navigate = useNavigate();
  const progressPercent = Math.min(100, Math.max(0, (raised / (target || 1)) * 100));

  // Landing page: safe redirect to campaigns list (don't deep-link to /campaigns/:id from here)
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        ...
        // Limit and normalize so imageUrl is set (backend may provide imageUrl)
