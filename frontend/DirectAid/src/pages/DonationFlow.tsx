...
import { LayoutDashboard, Receipt, FolderKanban, User, CreditCard, Bell, ArrowLeft, CheckCircle2, Heart, Zap, DollarSign, Lock, AlertCircle, Download, Smartphone, Copy, Check } from "lucide-react";
...
const DonationFlow = () => {
  ...
  const { selectedCampaign, createDonation } = useApp();

  const [currentStep, setCurrentStep] = useState<Step>("campaign");
  const currentUser = user;
  const donor = currentUser || { name: "Guest", email: "" }; // Fallback for guest

  // Redirect if no campaign selected (in real app)
  useEffect(() => {
    if (!selectedCampaign) {
      // For dev/demo if refreshing on this page, maybe just redirect or warn
  ...
  const handleLightningPayment = async () => {
    ...
    if (res && res.invoice) {
      setDonation(res); // Store full donation object (has id, etc)
      ...
      <div className="min-h-screen p-4 sm:p-6" style={{ backgroundColor: "#0a0e1a" }}>
        ...
        <div className="max-w-3xl mx-auto mb-8">
          ...
          <div className="mb-8 overflow-hidden">
            <h1 className="text-3xl sm:text-4xl font-bold mb-6" style={{ color: "#e0e0e0" }}>
              Make a Donation
            </h1>
        ...
        <div className="max-w-3xl mx-auto">
                  ...
                  <div className="space-y-4 mb-6">
                    <div>
                      ...
                      <p className="text-2xl font-bold" style={{ color: "#00ffff" }}>
                        ${(campaign.targetAmount / 100).toLocaleString()}
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold" style={{ color: "#00ffff" }}>
                          ${(campaign.amountRaised / 100).toLocaleString()}
                        </span>
                      </div>
                      ...
                      {formData.paymentMethod === "lightning" && (
                        <div className="space-y-4">
                          ...
                          <div className="text-center py-6">
                            <div className="w-full max-w-[280px] aspect-square mx-auto mb-4 rounded-lg flex items-center justify-center border-2 overflow-hidden" style={{ backgroundColor: "#ffffff", borderColor: "#00ffff" }}>
                              <div className="text-center p-4 w-full">
                                ...
                                <div className="grid grid-cols-8 gap-1 w-full max-w-[160px] mx-auto">
                                  {Array.from({ length: 64 }).map((_, i) => (
                                    <div key={i} className="aspect-square" style={{ backgroundColor: Math.random() > 0.5 ? "#0a0e1a" : "#ffffff" }} />
                                  ))}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="max-w-md mx-auto px-4">
                              ...
                              <div className="flex gap-2">
                                ...
                                >
                                  {invoiceCopied ? <Check className="w-4 h-4 sm:w-5 sm:h-5" /> : <Copy className="w-4 h-4 sm:w-5 sm:h-5" />}
                                </button>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="text-center">
                            <Button
                              ...
                              style={{ backgroundColor: "#00ffff", color: "#0a0e1a" }}
                            >
                              {paymentStatus === "sending" ? "Generating Invoice..." : "Pay with Lightning"}
                            </Button>
                          </div>
                          ...
                          {paymentStatus === "waiting" && (
                            <div className="text-center mt-4">
                              <p className="text-sm mb-2" style={{ color: "#e0e0e0" }}>Waiting for payment...</p>
                              <Button onClick={verifyPayment} className="rounded-full bg-green-600 text-white">
                                I've Paid
                              </Button>
                            </div>
                          )}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="text-center py-6">
                            <div className="w-full max-w-[280px] aspect-square mx-auto mb-4 rounded-lg flex items-center justify-center border-2 overflow-hidden" style={{ backgroundColor: "#ffffff", borderColor: "#00ffff" }}>
                              <div className="text-center p-4 w-full">
                                ...
                                <div className="grid grid-cols-8 gap-1 w-full max-w-[160px] mx-auto">
                                  {Array.from({ length: 64 }).map((_, i) => (
                                    <div key={i} className="aspect-square" style={{ backgroundColor: Math.random() > 0.6 ? "#0a0e1a" : "#ffffff" }} />
                                  ))}
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="text-center">
                            <Button
                              ...
                              style={{ backgroundColor: "#00ffff", color: "#0a0e1a" }}
                            >
                              {paymentStatus === "sending" ? "Generating Address..." : "I've Sent the Payment"}
                            </Button>
                          </div>
          ...
          {/* Step 5: Receipt */}
          {
            currentStep === "receipt" && donation && (
              <Card className="p-4 sm:p-8 text-center max-w-2xl mx-auto" style={{ backgroundColor: "#1a1f2e", borderColor: "#00ffff" }}>
                <CheckCircle2 className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4" style={{ color: "#00ffff" }} />
                <h2 className="text-xl sm:text-3xl font-bold mb-2 px-4" style={{ color: "#e0e0e0" }}>
                  Donation Successful!
                </h2>
                <p style={{ color: "#e0e0e0", opacity: 0.7 }} className="text-sm sm:text-base mb-6 max-w-md mx-auto px-4">
                ...
                {/* Receipt Details */}
                <div className="p-4 rounded-lg mb-6 text-left space-y-4 border-2 mx-1 sm:mx-0" style={{ backgroundColor: "#0a0e1a", borderColor: "#00ffff" }}>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider mb-1" style={{ color: "#e0e0e0", opacity: 0.6 }}>
                      Donation ID
                    </p>
                    <p className="font-mono text-xs sm:text-sm break-all" style={{ color: "#00ffff" }}>
                      {donation.id}
                    </p>
                  </div>
                  <div>
                    ...
                    <div className="flex flex-wrap items-baseline gap-2">
                      <p className="font-semibold text-lg sm:text-xl" style={{ color: "#00ffff" }}>
                        ${donationAmountUSD.toFixed(2)} USD
                      </p>
                      <span className="text-xs sm:text-sm" style={{ color: "#e5e7eb", opacity: 0.6 }}>
                        {formData.paymentMethod === "lightning" ? `(${donationAmountSats.toLocaleString()} sats)` : `(${donationAmountBTC} BTC)`}
                      </span>
                    </div>
                  </div>
                  <div>
                    ...
                    <p className="font-semibold text-sm sm:text-base" style={{ color: "#e0e0e0" }}>
                      {formData.paymentMethod === "lightning" ? "Lightning Network" : "Bitcoin (On-Chain)"}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider mb-1" style={{ color: "#e0e0e0", opacity: 0.6 }}>
                    ...
                    <p className="font-mono text-xs break-all" style={{ color: "#00ffff" }}>
                ...
                <div className="mb-8 text-left px-2 sm:px-0">
                  <h3 className="font-semibold mb-4 text-base sm:text-lg border-l-4 pl-3" style={{ color: "#e0e0e0", borderColor: "#00ffff" }}>
                    What Happens Next?
                  </h3>
                  <ol className="space-y-4 text-sm">
                    {[
                      ...
                      <li key={i} className="flex gap-4 items-start">
                        <span className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 font-bold text-xs" style={{ backgroundColor: "rgba(0, 255, 255, 0.2)", color: "#00ffff" }}>
                          {i + 1}
                        </span>
                        <span style={{ color: "#e0e0e0", opacity: 0.9 }} className="leading-relaxed">
                          {text}
                        </span>
                      </li>
                    ))}
