...
import { matchProviderToCampaign } from "../utils/matchingEngine";
...
const CampaignCreationWizard = () => {
  ...
  const campaign = {
    ...
    targetAmount: parseFloat(formData.targetAmount) * 250000,
    amountRaised: 0,
  ...
  const matchedProvider = matchProviderToCampaign(campaign, providers);

  // 3. Store system truth
  const campaignWithProvider = {
    ...campaign,
    providerId: matchedProvider?.id ?? null,
    provider: matchedProvider ?? null,
    status: matchedProvider ? "pending_approval" : "needs_provider",
  };

  // 4. Save campaign (context / mock service)
  addCampaign(campaignWithProvider);

  setNewCampaign(campaignWithProvider);
